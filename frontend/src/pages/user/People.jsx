...
const People = () => {
  ...
  const handleAddMember = async () => {
    ...
    try {
      ...
      // Gửi webhook cho từng thành viên được thêm
      for (const addedMember of addedMembers) {
        await fetch("https://n8n.quanliduan-pms.site/webhook/send_email", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
          ...
          triggerError(`Failed to send notification for ${addedMember.email}`);
        });
        await new Promise((resolve) => setTimeout(resolve, 500)); // Delay giữa các webhook
      }
